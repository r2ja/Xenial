...
const createTokens = (userId) => {
  const accessToken = jwt.sign({ user_id: Number(userId) }, config.jwtSecret, { expiresIn: '15m' });
  const refreshToken = jwt.sign({ user_id: Number(userId) }, config.refreshTokenSecret, { expiresIn: '7d' });
...
router.post("/register", async (req, res) => {
  ...
  try {
    ...
    // Create tokens
    const { accessToken, refreshToken } = createTokens(user.user_id);

    res.status(201).json({ 
      user: { user_id: Number(user.user_id), username: user.username, email: user.email },
      accessToken,
      refreshToken
    });
  } catch (err) {
